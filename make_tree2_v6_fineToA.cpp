...
int make_tree(string filename, int max_dist=3, int nentries=1000000000)
{
    ...
    ULong64_t globaltimefine = 0;
    ULong64_t prevglobaltime = 0;
    ULong64_t prevglobaltimefine = 0;
    Int_t pixelbits;
    Int_t longtimebits;
    Int_t diff;
    UInt_t data;
    ...
    Long64_t lmax_gap = 100;  // max gap of 50 ToA counts
    Float_t sumX, sumY;
    ...
    ULong64_t GlobalTimesFine[MAXHITS];
    ...
    ULong64_t tGlobalTimesFine[MAXHITS];
...
///////////////////////
    ...
    string rootfilename = newrootfile;
    ...
    rawtree->Branch("GlobalTimeFine", GlobalTimesFine, "GlobalTimeFine[Nhits]/l");
        ...
        if (retval == 1) {
            ...
            if ( header == 0x4  || header == 0x6 ) {   // packets with time information, old and new format: 0x4F and 0x6F
                ...
                if ( subheader == 0xF ) {
                    ...
                    //trigtime_fine = ((pixdata & 0x0000000000000FFF) >> 0 ) & 0xFFF;                    
                            ...
                            while (usedcnt < tnhits) {
                                ...
                                for (int ii=0; ii<tnhits; ii++) {
                                    if (tUsed[ii] != 0 ) {   // not yet used
                                        ...
                                        GlobalTimesFine[nhits] = tGlobalTimesFine[ii];
                                ...
                                while(tcnt < nhits) {
                                    for (int ii=0; ii<tnhits; ii++) {    // first hit is always used
                                        ...
                                        if (tUsed[ii] != 0 ) {   // not yet used
                                            ...
                                            if ( (distrow<max_dist) && (distrow>-1*max_dist) && (distcol<max_dist) && (distcol>-1*max_dist) ) {  // neigbour pixel 
                                                Rows[nhits] = tRows[ii];
                                                Cols[nhits] = tCols[ii];
                                                ToTs[nhits] = tToTs[ii];
                                                ToAs[nhits] = tToAs[ii];
